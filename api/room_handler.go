...
func NewHandleRoomStore(store *db.Store) *RoomHandler {
	return &RoomHandler{
		store: store,
	}
}
...
func (h *RoomHandler) isRoomAvailableForBooking(ctx context.Context, roomID primitive.ObjectID, params BookingParams) (bool, error) {
	filter := bson.M{
		"fromDate": bson.M{
			"$gte": params.FromDate,
		},
